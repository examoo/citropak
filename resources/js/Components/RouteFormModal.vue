...
watch(() => props.show, (newValue) => {
    if (newValue) {
        if (props.route) {
            ...
            form.status = props.route.status;
            form.distribution_id = props.route.distribution_id || '';
        } else {
            form.reset();
            form.status = 'active';
    ...
    <Modal :show="show" @close="closeModal" maxWidth="md">
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 mb-4 border-b pb-2">
                {{ isEditing ? 'Edit' : 'Add New' }} Route
            </h2>

            <form @submit.prevent="submit" class="space-y-4">
                <!-- Distribution Select (Only if Global View and Creating) -->
                <div v-if="!currentDistribution?.id && !isEditing">
                    <SearchableSelect v-model="form.distribution_id" label="Distribution (Optional)"
                        :options="distributions" option-value="id" option-label="value"
                        placeholder="Global (All Distributions)" :error="form.errors.distribution_id" />
                </div>
                ...
                <div>
                    ...
                    <TextInput v-model="form.name" type="text" class="mt-1 block w-full"
                        ...
                        placeholder="e.g. Route 1, North Area" />
