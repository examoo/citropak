...
watch(search, debounce((value) => {
    router.get(route('product-categories.index'), { search: value }, {
...
const submit = () => {
    ...
    if (isEditing.value) {
        form.put(route('product-categories.update', editingId.value), { onSuccess: () => closeModal() });
    } else {
        form.post(route('product-categories.store'), { onSuccess: () => closeModal() });
    }
};
...
const deleteItem = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('product-categories.destroy', item.id), {
                preserveScroll: true,
...
<template>
    ...
    <DashboardLayout>
        <div class="space-y-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    <div class="relative">
                        <input v-model="search" type="text" placeholder="Search..." class="pl-10 pr-4 py-2.5 rounded-xl border-gray-200 text-sm focus:border-orange-500 focus:ring-orange-500 w-64 shadow-sm">
                        <svg class="w-5 h-5 text-gray-400 absolute left-3 top-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    </div>
                    <button @click="openModal()" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-orange-600 to-amber-600 text-white rounded-xl font-medium shadow-lg shadow-orange-500/30 hover:shadow-xl transition-all duration-200 hover:-translate-y-0.5">
                ...
                <table class="w-full text-left text-sm text-gray-600">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="item in categories.data" :key="item.id" class="hover:bg-gray-50/50">
                            ...
                            <td class="px-6 py-4">
                                <span :class="['px-2 py-1 rounded-full text-xs font-medium', item.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-700']">{{ item.status.toUpperCase() }}</span>
                            </td>
                        ...
                        <tr v-if="categories.data.length === 0"><td colspan="5" class="px-6 py-12 text-center text-gray-500">No product categories found.</td></tr>
                    </tbody>
                </table>
        ...
        <Modal :show="isModalOpen" @close="closeModal" maxWidth="md">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 mb-4 border-b pb-2">{{ isEditing ? 'Edit' : 'Add New' }} Product Category</h2>
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div class="flex justify-end gap-3 mt-6 pt-4 border-t">
                        ...
                        <PrimaryButton :disabled="form.processing" class="bg-gradient-to-r from-orange-600 to-amber-600 border-0">{{ form.processing ? 'Saving...' : (isEditing ? 'Update' : 'Create') }}</PrimaryButton>
                    </div>
                </form>
            </div>
        </Modal>
