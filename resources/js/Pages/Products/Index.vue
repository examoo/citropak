...
watch(() => form.packing_id, (newPackingId) => {
    if (newPackingId) {
        ...
        if (selectedPacking && selectedPacking.conversion) {
            form.pieces_per_packing = parseInt(selectedPacking.conversion) || 1;
        }
    }
});
...
<template>
    ...
    <DashboardLayout>
            ...
            <Modal :show="isImportModalOpen" @close="closeImportModal" maxWidth="md">
                <div class="p-6">
                    ...
                    <div class="mt-6 flex justify-end gap-3">
                        ...
                        <PrimaryButton @click="handleImport" :disabled="importProcessing"
                    ...
                    <table class="w-full text-left text-sm text-gray-600">
                        <thead class="bg-gray-50/50 text-xs uppercase font-semibold text-gray-500">
                            <tr>
                                ...
                                <th @click="handleSort('unit_price')"
                                    class="px-6 py-4 cursor-pointer hover:text-emerald-600 transition-colors">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="product in products.data" :key="product.id"
                                ...
                                <td class="px-6 py-4 text-gray-500">
                                    <span class="px-2 py-1 bg-gray-100 rounded text-xs">{{ product.brand?.name || 'N/A'
                                    }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-gray-500">
                                    <span class="px-2 py-1 bg-purple-50 text-purple-700 rounded text-xs">{{
                                        product.product_type?.name || 'N/A' }}</span>
                                </td>
                                <td class="px-6 py-4 text-gray-500">
                ...
                <form @submit.prevent="submit" class="space-y-4 max-h-[70vh] overflow-y-auto pr-2">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <div v-if="form.errors.dms_code" class="text-xs text-red-600 mt-1">{{ form.errors.dms_code
                            }}</div>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="flex gap-2 mt-1">
                                <div class="flex-1">
                                    <SearchableSelect v-model="form.brand_id" :options="brands" option-value="id"
                                        option-label="name" placeholder="Select Brand" :error="form.errors.brand_id" />
                                </div>
                        ...
                        <div>
                            ...
                            <div class="flex gap-2 mt-1">
                                <div class="flex-1">
                                    <SearchableSelect v-model="form.type_id" :options="types" option-value="id"
                                        option-label="name" placeholder="Select Type" :error="form.errors.type_id" />
                                </div>
                                <button type="button" @click="isTypeModalOpen = true"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <div v-if="form.errors.reorder_level" class="text-xs text-red-600 mt-1">{{
                                form.errors.reorder_level }}</div>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <div class="flex gap-2 mt-1">
                                <div class="flex-1">
                                    <SearchableSelect v-model="form.packing_id" :options="packings" option-value="id"
                                        option-label="name" placeholder="Select Packing"
                                        :error="form.errors.packing_id" />
                                </div>
                                <button type="button" @click="isPackingModalOpen = true"
                        ...
                        <div>
                            ...
                            <TextInput v-model="form.pieces_per_packing" type="number" min="1" step="1"
                                class="mt-1 block w-full" />
                            <div v-if="form.errors.pieces_per_packing" class="text-xs text-red-600 mt-1">{{
                                form.errors.pieces_per_packing }}</div>
                        </div>
                    </div>
            ...
            <div class="p-6">
                ...
                <form @submit.prevent="submitBrand" class="space-y-4">
                    <div>
                        ...
                        <TextInput v-model="brandForm.name" type="text" class="mt-1 block w-full"
                            :class="{ 'border-red-500': brandForm.errors.name }" />
                        <div v-if="brandForm.errors.name" class="text-xs text-red-600 mt-1">{{ brandForm.errors.name }}
                        </div>
                    </div>
                    <div>
                        <InputLabel value="Status" />
                        <select v-model="brandForm.status"
                    ...
                    <div class="flex justify-end gap-3 mt-6 pt-4 border-t">
                        ...
                        <PrimaryButton :disabled="brandForm.processing"
                            class="bg-gradient-to-r from-emerald-600 to-teal-600 border-0">{{ brandForm.processing ?
                                'Saving...' : 'Create Brand' }}</PrimaryButton>
                    </div>
                </form>
            </div>
        </Modal>
            ...
            <div class="p-6">
                ...
                <form @submit.prevent="submitType" class="space-y-4">
                    <div>
                        ...
                        <TextInput v-model="typeForm.name" type="text" class="mt-1 block w-full"
                    ...
                    <div class="flex justify-end gap-3 mt-6 pt-4 border-t">
                        ...
                        <PrimaryButton :disabled="typeForm.processing"
                            class="bg-gradient-to-r from-purple-600 to-indigo-600 border-0">{{ typeForm.processing ?
                                'Saving...' : 'Create Type' }}</PrimaryButton>
                    </div>
                </form>
            </div>
        </Modal>
        ...
        <!-- Packing Modal -->
        <FormModal :show="isPackingModalOpen" @close="isPackingModalOpen = false" @saved="handlePackingSaved" />
    </DashboardLayout>
</template>
