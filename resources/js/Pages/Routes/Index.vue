...
const submit = () => {
    if (!form.name) { form.setError('name', 'Required.'); return; }
    if (isEditing.value) {
        form.put(route('routes.update', editingId.value), { onSuccess: () => closeModal() });
    } else {
        form.post(route('routes.store'), { onSuccess: () => closeModal() });
    }
};

const deleteItem = (item) => {
    Swal.fire({ title: 'Delete?', text: `Delete "${item.name}"?`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33' })
        .then((r) => { if (r.isConfirmed) router.delete(route('routes.destroy', item.id), { preserveScroll: true }); });
};
</script>
    ...
    <DashboardLayout>
        <div class="space-y-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div><h1 class="text-2xl font-bold text-gray-900">Routes</h1><p class="text-gray-500 mt-1">Manage delivery routes</p></div>
