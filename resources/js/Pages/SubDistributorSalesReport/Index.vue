...
const applyFilters = () => {
    router.get(route('sub-distributor-sales-reports.index'), filters.value, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
<template>
    <Head title="Sub-Distributor Sales Report" />

    <DashboardLayout>
        <template #header>
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">Sub-Distributor Sales & Stock Report</h2>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        ...
                        <div>
                            <InputLabel value="Sub Distributor" />
                                    ...
                                    <table class="min-w-full divide-y divide-gray-200">
                                        <thead class="bg-gray-50">
                                            <tr>
                                                ...
                                                <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Sold Qty (Stock Out)</th>
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200">
                                            <tr v-for="item in items" :key="item.product_id" class="hover:bg-gray-50">
                                                ...
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-semibold">{{ item.total_quantity }}</td>
                                            ...
                                            <!-- Brand Subtotal -->
                                            <tr class="bg-indigo-50/30 font-semibold">
                                                <td colspan="2" class="px-6 py-3 text-right text-indigo-800">Total {{ brandName }}:</td>
                                                <td class="px-6 py-3 text-right text-indigo-800">{{ items.reduce((sum, i) => sum + i.total_quantity, 0) }}</td>
                        ...
                        <div v-if="Object.keys(groupedData).length > 0" class="mt-8 border-t-4 border-gray-200 pt-6">
                            ...
                            <dl class="grid grid-cols-1 gap-5 sm:grid-cols-5">
                                <div class="px-4 py-5 bg-gray-50 shadow rounded-lg overflow-hidden sm:p-6">
                                    <dt class="text-sm font-medium text-gray-500 truncate">Total Quantity Sold</dt>
                                    <dd class="mt-1 text-3xl font-semibold text-gray-900">{{ totals.quantity }}</dd>
                                </div>
