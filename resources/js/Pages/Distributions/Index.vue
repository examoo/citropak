...
watch(search, debounce((value) => {
    router.get(route('distributions.index'), { search: value }, {
        preserveState: true,
        preserveScroll: true,
        replace: true
    });
...
const submit = () => {
    ...
    if (isEditing.value) {
        form.put(route('distributions.update', editingId.value), {
...
const deleteItem = (item) => {
    Swal.fire({
        title: 'Are you sure?',
        text: `Delete distribution "${item.name}"?`,
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            router.delete(route('distributions.destroy', item.id), {
                preserveScroll: true,
                onSuccess: () => {
                    Swal.fire('Deleted!', 'Distribution has been deleted.', 'success');
                }
            });
        }
    })
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-500 mt-1">Manage distribution centers (DMS, TMS, LMS)</p>
                </div>
        ...
        <Modal :show="isModalOpen" @close="closeModal" maxWidth="md">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 mb-4 border-b pb-2">
                    {{ isEditing ? 'Edit' : 'Add New' }} Distribution
                </h2>

                <form @submit.prevent="submit" class="space-y-4">
                    <div>
                        <InputLabel value="Distribution Name" />
                        <TextInput 
                            v-model="form.name" 
                            type="text" 
                            class="mt-1 block w-full" 
                            :class="{ 'border-red-500 focus:border-red-500 focus:ring-red-500': form.errors.name }"
                            placeholder="e.g. Distribution Management System"
                        />
