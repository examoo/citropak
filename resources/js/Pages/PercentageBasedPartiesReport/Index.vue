                    ...
                    <div>
                        ...
                        <div class="flex gap-2 mt-1">
                            <SearchableSelect
                                ...
                                option-value="id"
                                option-label="name"
                                placeholder="Select a brand to add..."
                                class="flex-1"
                                @update:modelValue="addBrand"
                            />
                        </div>
                        <!-- Selected Brand Tags -->
                        <div v-if="selectedBrandNames.length > 0" class="flex flex-wrap gap-2 mt-3">
                            <div
                                v-for="brand in selectedBrandNames"
                                :key="brand.id"
                                class="inline-flex items-center gap-2 px-3 py-1 bg-indigo-100 text-indigo-700 rounded-full text-sm"
                            >
                                <span>{{ brand.name }}</span>
                                <button @click="removeBrand(brand.id)" class="hover:text-indigo-900 font-bold">×</button>
                            </div>
                        </div>
                    ...
                    <div>
                        ...
                        <div v-if="selectedCustomersNames.length > 0" class="flex flex-wrap gap-2 mt-3">
                            ...
                            >
                                ...
                                <button @click="removeCustomer(customer.id)" class="hover:text-blue-900 font-bold">×</button>
                            </div>
                            <button
                    ...
                    <table class="w-full text-left text-sm">
                        <thead class="bg-gray-50 text-xs uppercase font-semibold text-gray-500">
                            <tr>
                                ...
                                <!-- Brand columns -->
                                <th
                                    v-for="brand in brands.filter(b => filters.brand_ids.length === 0 || filters.brand_ids.map(Number).includes(b.id))"
                                    :key="brand.id"
                                    class="px-3 py-2 text-center bg-blue-50 min-w-[130px]"
                                >
                                    {{ brand.name }}
                                </th>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="party in reportData" :key="party.customer_id" class="hover:bg-gray-50/50">
                                <td class="px-3 py-2 font-medium text-gray-700 sticky left-0 bg-white">{{ party.customer_code }}</td>
                                <td class="px-3 py-2 sticky left-20 bg-white whitespace-nowrap">{{ party.shop_name }}</td>
                                <td class="px-3 py-2 text-center font-semibold text-blue-600">{{ party.percentage || 0 }}%</td>
...
<style scoped>
@media print {
    nav, header, aside, .fixed, .sticky, button, .no-print, .print\:hidden {
