        ...
        <div class="space-y-6 max-w-6xl">
            ...
            <form @submit.prevent="submit" class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <p v-if="form.invoice_type === 'damage'" class="text-xs text-red-600 mt-1">All prices will be set to zero</p>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea v-model="form.notes" rows="2" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm"></textarea>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-7 gap-3 items-end">
                        ...
                        <div>
                            ...
                            <select v-model="newItem.scheme_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-emerald-500 focus:ring-emerald-500 text-sm">
                                ...
                                <option v-for="s in productSchemes" :key="s.id" :value="s.id">{{ s.discount_value }}{{ s.discount_type === 'percentage' ? '%' : ' Rs' }}</option>
                            </select>
                        </div>
                        <div>
                            <button type="button" @click="addItem" :disabled="!newItem.product_id || newItem.total_pieces <= 0" class="w-full px-4 py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50">+ Add</button>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                <tr v-for="(item, index) in form.items" :key="index">
                                    ...
                                    <td class="px-4 py-3 text-right font-semibold text-emerald-600">{{ formatAmount((item.total_pieces * item.price) - item.scheme_discount) }}</td>
                                    <td class="px-4 py-3">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <PrimaryButton :disabled="form.processing || form.items.length === 0" class="bg-gradient-to-r from-emerald-600 to-teal-600 border-0">
