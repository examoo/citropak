...
watch(search, debounce((value) => {
    router.get(route('packings.index'), { search: value }, {
        preserveState: true,
        preserveScroll: true,
        replace: true
    });
...
const deleteItem = (item) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('packings.destroy', item.id));
        }
    });
};
</script>
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-500 mt-1">Manage product packing types and conversions.</p>
                </div>
                <button 
                    @click="openModal()" 
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            <tr v-for="item in packings.data" :key="item.id" class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ item.name }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-900">{{ item.conversion }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <!-- Pagination -->
                <div v-if="packings.links.length > 3" class="px-4 py-3 border-t border-gray-200 bg-gray-50">
