...
const submit = () => {
    if (!form.product_id || form.quantity <= 0) {
        Swal.fire('Error', 'Select a stock and enter quantity', 'error');
        return;
    }
    if (isEditing.value) {
        form.put(route('closing-stocks.update', editingId.value), { onSuccess: () => closeModal() });
    } else {
        form.post(route('closing-stocks.store'), { onSuccess: () => closeModal() });
    }
};

const postStock = (item) => {
    Swal.fire({
        title: 'Post Closing Stock?',
        text: 'This will deduct the stock from inventory. This action cannot be undone.',
        icon: 'question',
        ...
        if (result.isConfirmed) {
            router.post(route('closing-stocks.post', item.id), {}, {
                preserveScroll: true,
                onSuccess: () => Swal.fire('Posted!', 'Closing stock deducted from inventory.', 'success')
            });
        }
    });
};
...
const deleteItem = (item) => {
    if (item.status === 'posted') {
        Swal.fire('Error', 'Cannot delete posted closing stock', 'error');
        return;
    }
    Swal.fire({
        title: 'Delete Closing Stock?',
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            router.delete(route('closing-stocks.destroy', item.id), {
                preserveScroll: true,
                onSuccess: () => Swal.fire('Deleted!', 'Closing stock deleted.', 'success')
            });
        }
    });
};
...
const revertItem = (item) => {
    Swal.fire({
        title: 'Revert Closing Stock?',
        text: 'This will change status back to Draft. No inventory changes will occur.',
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            router.post(route('closing-stocks.revert', item.id), {}, {
                preserveScroll: true,
                onSuccess: () => Swal.fire('Reverted!', 'Closing stock reverted to draft.', 'success')
            });
        }
    });
};
...
const convertFromStocks = () => {
    Swal.fire({
        ...
        text: 'This will create draft closing stocks from all available stocks. You can review and post them individually.',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#f59e0b',
        ...
        if (result.isConfirmed) {
            router.post(route('closing-stocks.convert'), {}, {
                preserveScroll: true,
                onSuccess: () => Swal.fire('Converted!', 'Available stocks converted to closing stocks.', 'success')
            });
        }
    });
};
    ...
    <DashboardLayout>
        <div class="space-y-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Closing Stocks</h1>
                    <p class="text-gray-500 mt-1">Record end-of-period stock balances</p>
                </div>
                ...
                <table class="w-full text-left text-sm text-gray-600">
                    <thead class="bg-gray-50/50 text-xs uppercase font-semibold text-gray-500">
                        <tr>
                            ...
                            <th class="px-6 py-4">Closing Qty</th>
                            <th class="px-6 py-4">Status</th>
                            <th class="px-6 py-4 text-right">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="item in closingStocks.data" :key="item.id" class="hover:bg-gray-50/50">
                            <td class="px-6 py-4">
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div v-if="selectedProduct" class="bg-white rounded-lg p-4 border">
                        ...
                        <div class="grid grid-cols-4 md:grid-cols-8 gap-2 text-xs">
                            ...
                            <div class="p-2 bg-amber-50 rounded"><span class="text-amber-600">Value:</span> {{ (form.quantity * form.unit_cost).toFixed(2) }}</div>
                        </div>
                    </div>
