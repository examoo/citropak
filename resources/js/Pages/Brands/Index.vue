...
const submit = () => {
    ...
    if (isEditing.value) {
        form.put(route('brands.update', editingId.value), { onSuccess: () => closeModal() });
    } else {
        form.post(route('brands.store'), { onSuccess: () => closeModal() });
    }
};
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('brands.destroy', item.id), {
                preserveScroll: true,
                onSuccess: () => Swal.fire('Deleted!', 'Brand deleted.', 'success')
            });
        }
    });
};
        ...
        <div class="space-y-6">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    ...
                    <p class="text-gray-500 mt-1">Manage product brands (Global)</p>
                </div>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <table class="w-full text-left text-sm text-gray-600">
                    ...
                    <tbody class="divide-y divide-gray-100">
                        <tr v-for="item in brands.data" :key="item.id" class="hover:bg-gray-50/50">
                        ...
                        <tr v-if="brands.data.length === 0"><td colspan="4" class="px-6 py-12 text-center text-gray-500">No brands found.</td></tr>
                    </tbody>
                </table>
                <div class="p-4 border-t border-gray-100 bg-gray-50/50"><Pagination :links="brands.links" /></div>
            </div>
        </div>

        <Modal :show="isModalOpen" @close="closeModal" maxWidth="md">
            <div class="p-6">
                <h2 class="text-lg font-medium text-gray-900 mb-4 border-b pb-2">{{ isEditing ? 'Edit' : 'Add New' }} Brand</h2>
